...
function encodeFromBuffers(timestamp, value, seriesBuf, tagBufs) {
...
export function createEventBinaryEncoder(series, tags) {
    const seriesBuf = toUtf8(series, "series");
    const tagBufs = tags.map((tag) => toUtf8(tag, "tag"));
    return (timestamp, value) => encodeFromBuffers(timestamp, value, seriesBuf, tagBufs);
}
export function encodeEventBinary(event) {
...
export function decodeEventBinary(buf) {
    ...
    offset += 2;
    if (offset + seriesLen > buf.length)
        return null;
