...
pub fn initHostFunctions(store: *zware.Store, context_ptr: usize) !void {
    try store.exposeHostFunction("env", "u_poll_handle", u_poll_handle, context_ptr, &[_]zware.ValType{.I64}, &[_]zware.ValType{.I32});
    try store.exposeHostFunction("env", "u_free_handle", u_free_handle, context_ptr, &[_]zware.ValType{.I64}, &[_]zware.ValType{.I32});
    try store.exposeHostFunction("env", "u_query_live", u_query_live, context_ptr, &[_]zware.ValType{.I32}, &[_]zware.ValType{.I64});
    try store.exposeHostFunction("env", "u_query_history", u_query_history, context_ptr, &[_]zware.ValType{.I32}, &[_]zware.ValType{.F64});
    ...
    // done
    try store.exposeHostFunction("env", "u_writeback_ws", u_writeback_ws, context_ptr, &[_]zware.ValType{ .I64, .I32 }, &[_]zware.ValType{.I32});
...
fn parseWriteEventTags(
    ...
    tags_out: *std.ArrayList([]const u8),
) !struct { series: []const u8, tags: []const []const u8 } {
    var series: []const u8 = "temp";
    tags_out.clearRetainingCapacity();

    var iter = std.mem.splitScalar(u8, tags_csv, ',');
    while (iter.next()) |raw| {
        ...
        try tags_out.append(allocator, token);
    }

    return .{ .series = series, .tags = tags_out.items };
}
