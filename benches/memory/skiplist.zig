...
pub fn run(allocator: Allocator, io: std.Io) !void {
    ...
    var list = try SkipList.init(allocator, @intCast(std.Io.Clock.real.now(io).toMicroseconds()));
    defer list.deinit();
    ...
    const ss = std.Io.Clock.awake.now(io);
    var found: usize = 0;
    for (0..n_searches) |_| {
        const ki = random.intRangeAtMost(usize, 0, n_inserts - 1);
        if (list.search(&keys[ki]) != null) found += 1;
    }
    const search_elapsed_ns: u64 = @intCast(ss.untilNow(io, .awake).toNanoseconds());
    const search_ns = @as(f64, @floatFromInt(search_elapsed_ns)) / @as(f64, @floatFromInt(n_searches));
