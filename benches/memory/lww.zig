...
pub fn run(allocator: Allocator, io: std.Io) !void {
    ...
    var wall_time: i64 = @intCast(std.Io.Clock.real.now(io).toMilliseconds());

    for (0..n_puts) |i| {
        if (i % sync_interval == 0) {
            wall_time = @intCast(std.Io.Clock.real.now(io).toMilliseconds());
        }
    ...
    const gs = std.Io.Clock.awake.now(io);
    var hits: usize = 0;
    for (key_slices) |k| {
        if (reg.get(k) != null) hits += 1;
    }
    const get_elapsed_ns: u64 = @intCast(gs.untilNow(io, .awake).toNanoseconds());
    const get_ns = @as(f64, @floatFromInt(get_elapsed_ns)) / @as(f64, @floatFromInt(n_keys));
